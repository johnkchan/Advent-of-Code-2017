key = '''nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
print(sum([1 for line in key.split("\n") if len([item for item, count in collections.Counter([''.join(sorted(
    x)) for x in line.split()]).items() if count > 1]) == 0]))
